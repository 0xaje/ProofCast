...
import { Bell, Compass, FileCheck2, GitCompareArrows, Menu, Radio, Search, Settings2, WalletCards, X } from "lucide-react";
import { ReactNode, useState } from "react";

const navItems = [
  { href: "/", label: "Command center", icon: Compass },
...
export function ProbabilityBand({ market, model, you, compact = false }: { market: number; model: number; you: number; compact?: boolean }) {
  ...
  return <div className={compact ? "space-y-3" : "space-y-5"}>{rows.map(([label, value, bar, text]) => <div key={label as string}><div className="mb-2 flex items-center justify-between text-[11px]"><span className="text-[#a8b0bd]">{label as string}</span><span className={`font-mono font-semibold ${text}`}>{value as number}% UP</span></div><div className="h-2 overflow-hidden rounded-full bg-white/[0.07]"><div className={`h-full rounded-full ${bar}`} style={{ width: `${value as number}%` }} /></div></div>)}</div>;
}

function RailContent({ onNavigate }: { onNavigate?: () => void }) {
  const [location] = useLocation();
  return <>
    <div className="flex items-center gap-3 px-5 py-6"><img src="/manus-storage/proofcast-mark_6cbda10a.png" alt="Proofcast proof seal" className="h-9 w-9 rounded-xl border border-[#d7f36b]/25 bg-[#d7f36b]/10" /><div><div className="font-display text-[18px] font-semibold tracking-[-0.07em] text-white">proof<span className="text-[#d7f36b]">cast</span></div><div className="text-[9px] uppercase tracking-[0.22em] text-[#6f7b8f]">Signal room / 01</div></div></div>
    <div className="px-4 pt-8"><div className="px-3 pb-3 text-[9px] font-bold uppercase tracking-[0.22em] text-[#697487]">Workspace</div><nav className="space-y-1">{navItems.map(({ href, label, icon: Icon }) => { const active = href === "/" ? location === "/" : location.startsWith(href); return <Link key={href} href={href} onClick={onNavigate} className={`flex items-center justify-between rounded-xl px-3 py-3 text-[12px] transition-all duration-200 ${active ? "bg-white/[0.08] text-white shadow-[inset_0_0_0_1px_rgba(255,255,255,0.03)]" : "text-[#8b96a8] hover:bg-white/[0.04] hover:text-white"}`}><span className="flex items-center gap-3"><Icon size={16} className={active ? "text-[#d7f36b]" : "text-[#687387]"} />{label}</span>{active && <span className="text-[#d7f36b]">›</span>}</Link>; })}</nav></div>
    <div className="mt-auto p-4"><div className="rounded-2xl border border-[#d7f36b]/15 bg-[#d7f36b]/[0.035] p-4"><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.17em] text-[#d7f36b]"><Radio size={13} /> Connection</div><div className="mt-3 text-[12px] font-medium text-white">Live data unavailable</div><div className="mt-1 text-[11px] leading-5 text-[#8993a4]">DreamDEX integration has not been connected to this workspace.</div><button disabled className="mt-4 flex w-full cursor-not-allowed items-center justify-center gap-2 rounded-lg bg-[#d7f36b]/35 py-2.5 text-[11px] font-bold text-[#10140d]/80"><WalletCards size={14} /> Integration pending</button></div><div className="mt-4 flex items-center gap-3 border-t border-white/[0.07] pt-4"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#202937] text-[10px] font-bold text-[#d7f36b]">AO</div><div><div className="text-[11px] font-semibold text-white">Operator workspace</div><div className="text-[10px] text-[#677285]">Read-only mode</div></div><Settings2 size={14} className="ml-auto text-[#677285]" /></div></div>
  </>;
}

export function SignalShell({ children }: { children: ReactNode }) {
  const [mobileOpen, setMobileOpen] = useState(false);
  return <div className="min-h-screen bg-[#080b10] text-[#f5f6f2] selection:bg-[#d7f36b] selection:text-[#10140d]"><div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_78%_0%,rgba(45,68,133,0.16),transparent_28%),linear-gradient(90deg,rgba(255,255,255,0.015)_1px,transparent_1px)] [background-size:auto,72px_100%]" />
