...
export default function ProofProfile() {
  ...
  return <SignalShell><div className="pi-workspace">
    ...
    <section className="pi-score-strip"><div><span>Calibration</span><b>{metrics.isLoading ? "…" : metrics.data?.calibrationStatus === "READY" ? "Ready" : "—"}</b><i>{metrics.data ? `${metrics.data.verifiedCount} verified / ${metrics.data.minimumSampleSize} minimum` : "Requires verified outcomes"}</i></div><div><span>Directional accuracy</span><b>{metrics.data?.directionalAccuracyPct == null ? "—" : `${metrics.data.directionalAccuracyPct.toFixed(1)}%`}</b><i>Verified outcomes only</i></div><div><span>Mean Brier score</span><b>{metrics.data?.meanBrierScoreBps == null ? "—" : `${(metrics.data.meanBrierScoreBps / 100).toFixed(1)}%`}</b><i>Lower is better · unresolved excluded</i></div></section>
    {auth.isAuthenticated && metrics.isError ? <section className="pi-panel pi-calibration-panel" data-testid="calibration-metrics" role="alert"><div className="pi-panel-head"><div><div className="pi-kicker"><span>Calibration bins</span> Verified evidence only</div><h2>Calibration metrics unavailable.</h2></div><StatusChip tone="unavailable">Error</StatusChip></div><p className="pi-lock-note">Proofcast could not calculate verified metrics. No score is substituted.</p><button type="button" className="pi-action" onClick={() => metrics.refetch()}>Retry metrics <ArrowUpRight size={15} /></button></section> : auth.isAuthenticated && metrics.data && <section className="pi-panel pi-calibration-panel" data-testid="calibration-metrics"><div className="pi-panel-head"><div><div className="pi-kicker"><span>Calibration bins</span> Verified evidence only</div><h2>{metrics.data.calibrationStatus === "READY" ? "Enough resolved history to inspect calibration." : "Calibration is not claimed yet."}</h2></div><StatusChip tone={metrics.data.calibrationStatus === "READY" ? "live" : "snapshot"}>{metrics.data.calibrationStatus === "READY" ? "Ready" : "Insufficient sample"}</StatusChip></div><div className="pi-detail-grid">{metrics.data.bins.filter(bin => bin.count > 0).map(bin => <div key={bin.lowerBps}><span>{(bin.lowerBps / 100).toFixed(0)}–{(bin.upperBps / 100).toFixed(0)}% forecasts · {bin.count} receipt{bin.count === 1 ? "" : "s"}</span><b>Predicted {(bin.predictedBps / 100).toFixed(1)}% / observed {(bin.observedBps / 100).toFixed(1)}%</b></div>)}</div><p className="pi-lock-note">{metrics.data.excludedCount} receipt{metrics.data.excludedCount === 1 ? "" : "s"} excluded because evidence is unresolved, rejected, or void.</p></section>}
    {auth.isAuthenticated && metrics.data && <section className="pi-panel pi-trend-panel" data-testid="calibration-trend"><div className="pi-panel-head"><div><div className="pi-kicker"><span>Verified history</span> Trend view</div><h2>{metrics.data.trend.length >= metrics.data.minimumSampleSize ? "Calibration progression, not a performance claim." : `Trend unlocks after ${metrics.data.minimumSampleSize} verified outcomes.`}</h2></div><StatusChip tone={metrics.data.trend.length >= metrics.data.minimumSampleSize ? "live" : "snapshot"}>{metrics.data.trend.length >= metrics.data.minimumSampleSize ? "History ready" : `${metrics.data.trend.length}/${metrics.data.minimumSampleSize}`}</StatusChip></div><button type="button" className="pi-action pi-export-action" onClick={downloadCalibrationCsv} disabled={exportCsv.isFetching}>{exportCsv.isFetching ? "Preparing CSV…" : "Download verified history CSV"} <ArrowUpRight size={15} /></button>{exportCsv.error && <p className="pi-error-note" role="alert">Verified history export unavailable. No file was created.</p>}{metrics.data.trend.length >= metrics.data.minimumSampleSize ? <div className="pi-trend-grid"><div><div className="pi-trend-label"><span>Directional accuracy</span><b>{metrics.data.trend.at(-1)?.directionalAccuracyPct.toFixed(1)}%</b></div><svg viewBox="0 0 100 100" role="img" aria-label="Directional accuracy trend"><polyline points={trendLine(metrics.data.trend.map(point => point.directionalAccuracyPct))} fill="none" stroke="currentColor" strokeWidth="2" vectorEffect="non-scaling-stroke" /></svg></div><div><div className="pi-trend-label"><span>Mean Brier score</span><b>{percentFromBps(metrics.data.trend.at(-1)?.meanBrierScoreBps ?? 0)}</b></div><svg viewBox="0 0 100 100" role="img" aria-label="Mean Brier score trend"><polyline points={trendLine(metrics.data.trend.map(point => point.meanBrierScoreBps), 10_000)} fill="none" stroke="currentColor" strokeWidth="2" vectorEffect="non-scaling-stroke" /></svg></div></div> : <div className="pi-trend-empty"><Clock3 size={22} /><p>Proofcast will show chronological accuracy and Brier movement once the minimum verified sample is reached.</p></div>}</section>}
    {isAdmin && <section className="pi-panel pi-review-queue" data-testid="admin-review-queue"><div className="pi-panel-head"><div><div className="pi-kicker"><span>Administrator queue</span> Evidence review</div><h2>{reviewQueue.isLoading ? "Loading submitted evidence…" : `${reviewQueue.data?.length ?? 0} item${(reviewQueue.data?.length ?? 0) === 1 ? "" : "s"} awaiting review`}</h2></div><StatusChip tone={reviewQueue.isError ? "unavailable" : "snapshot"}>{reviewQueue.isError ? "Error" : "Admin only"}</StatusChip></div>{reviewQueue.isError ? <div className="pi-ledger-empty" role="alert"><b>Review queue unavailable.</b><p>No evidence has been reviewed from this failed request.</p><button className="pi-action" type="button" onClick={() => reviewQueue.refetch()}>Retry queue</button></div> : reviewQueue.data?.length ? <div className="pi-review-list">{reviewQueue.data.map(item => <div className="pi-review-entry" key={item.resolution.id} data-testid={`review-row-${item.resolution.id}`}><div><b>Receipt #{item.receipt.id} · {item.resolution.outcome}</b><small>{item.resolution.evidenceSummary}</small><a href={item.resolution.sourceUrl} target="_blank" rel="noreferrer">{item.resolution.sourceUrl} <ArrowUpRight size={13} /></a></div><div className="pi-review-actions"><button type="button" className="pi-action" disabled={reviewEvidence.isPending} onClick={() => reviewEvidence.mutate({ resolutionId: item.resolution.id, status: "VERIFIED" })}>Verify</button><button type="button" className="pi-action pi-action-secondary" disabled={reviewEvidence.isPending} onClick={() => reviewEvidence.mutate({ resolutionId: item.resolution.id, status: "REJECTED" })}>Reject</button></div></div>)}</div> : <div className="pi-ledger-empty"><CheckCircle2 size={27} /><b>Queue clear.</b><p>Submitted evidence will appear here before it can affect scoring.</p></div>}{reviewEvidence.error && <p className="pi-error-note" role="alert">{reviewEvidence.error.message}</p>}</section>}
    ...
    <section className="pi-panel pi-calibration-note"><div><div className="pi-kicker"><span>Calibration standard</span> Evidence before score</div><h2>Significance starts with enough resolved data.</h2><p>Proofcast excludes unresolved, rejected, and void evidence. A calibration status becomes Ready only after the configured minimum verified sample.</p></div><div><Clock3 size={16} /> {metrics.data ? `${metrics.data.verifiedCount} verified receipt${metrics.data.verifiedCount === 1 ? "" : "s"}` : "Resolution data pending verification"}</div></section>
  </div></SignalShell>;
}
