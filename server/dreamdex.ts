...
export async function getDreamDexSnapshot(limit = 3): Promise<DreamDexSnapshot> {
  ...
  try {
    ...
    const selectedMarkets = liveMarkets.slice(0, limit);
    ...
    const now = Date.now();
    if (lastSuccessfulSnapshot?.asOf && now - lastSuccessfulSnapshot.asOf <= STALE_CACHE_MS) {
      return {
        ...lastSuccessfulSnapshot,
        state: "STALE",
        ageMs: now - lastSuccessfulSnapshot.asOf,
